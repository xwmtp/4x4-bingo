...
var BLACKOUT_PROFILE = {
    defaultMinimumSynergy: -8.5,
    defaultMaximumSynergy: 8.5,
    defaultMaximumIndividualSynergy: 4.5,
    defaultMaximumSpill: 3,
...
BingoGenerator.prototype.generatePopulationOrder = function () {
    ...
    var middle = [7, 10, 11].shuffled();
    populationOrder = populationOrder.concat(middle);

    //Finally add the rest of the squares
    var rest = [1, 2, 3, 4, 5, 8, 9, 12, 13, 14, 15, 16].shuffled();
    populationOrder = populationOrder.concat(rest);

    //Lastly, find location of difficulty 14,15,16 elements and put them out front
    for (var k = 14; k <= 16; k++) {
...
ootBingoGenerator = function (bingoList, opts) {
    ...
    if (!card) {
        console.log();
        console.log(`Couldn't generate a board after ${iterations} iterations`);
    }
