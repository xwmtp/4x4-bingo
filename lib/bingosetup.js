...
const DEV_MODE = true
const prefix = DEV_MODE ? '..' : '/beginner-bingo'
...
boardInfo.innerHTML = `
OoT Beginner Bingo <strong>${bingoOpts.version}</strong>&emsp;
...
document.querySelectorAll('#board-popout').forEach(el => el.addEventListener('click', () => {
  ...
  window.open(`${prefix}/board-popout.html?seed=${bingoOpts.seed}&mode=${bingoOpts.mode}&version=${bingoOpts.version}${bingoOpts.lang === 'jp' ? '&lang=jp' : ''}`, '_blank', `width=${settings.boardPopoutWidth}, height=${settings.boardPopoutHeight}, toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=no, resizable=no, copyhistory=no`);
}));
...
document.querySelectorAll('.popout').forEach(el => el.addEventListener('click', () => {
  ...
  window.open(`${prefix}/bingo-popout.html${bingoOpts.lang === 'jp' ? '?lang=jp' : ''}#${encodeURIComponent(`${name}=${items.join(';;;')}`)}`, '_blank', `toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=no, resizable=no, copyhistory=no, width=${settings.rowPopoutWidth}, height=${settings.rowPopoutHeight}`);
}));
